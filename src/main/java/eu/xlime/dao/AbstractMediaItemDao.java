...
public abstract class AbstractMediaItemDao implements MediaItemDao {
	...
	public List<MediaItem> findMediaItems(final List<String> urls) {
		 ...
		 */
		return weave(findNewsArticles(filterNews(urls)),
	...
	abstract protected List<String> findMediaItemsByDate(long dateFrom, long dateTo,
			int limit);
	
	private List<MediaItem> weave(List<? extends MediaItem>... lists) {
		...
		for (int i = 0; i < max; i++) {
			for (List<? extends MediaItem> list: lists) {
